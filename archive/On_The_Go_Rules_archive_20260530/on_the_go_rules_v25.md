...
Created: 2026-04-30 | Updated: 2026-05-20 | Status: Active | Version: v25
...
| on\_the\_go\_rules (active) | `Rules/on_the_go_rules_v25.md` | this file |
...
Apply ALL checks. Never mention what was removed. Never explain. A place only makes the list if it passes every single check.

### Check 1 — Is it open right now?
Look at today's hours. If closed today → drop it.

### Check 2 — Is it ACTUALLY SERVING FOOD right now?
This is not the same as being open. Many European restaurants, especially in Portugal, Spain, France, and Italy:
- Stop lunch service at 1:00–3:00pm
- Have a dead zone from ~3pm to 7pm where they are "open" but only serving drinks
- Don't start dinner until 7:00–7:30pm

**The test:** Take the current time from `user_time_v0`. Check if that time falls within an actual food service window. If a place is open 12–11pm but only serves food 12–3pm and 7–11pm, and it's 5:40pm → it fails. Drop it silently.

**Never send Dani somewhere that is open but not serving food.** This already happened once (Postigo do Carvão at 5:40pm). It cannot happen again.
...
- Always verify the actual menu — never rely on category
- If good non-seafood dishes exist (steak, duck, lamb, pork, pasta, veal) → include, show only those dishes
- If seafood dominates with minimal alternatives → drop silently
...
- **v25 (2026-05-20):** Food service check hardened into its own mandatory step with explicit test. "Open" ≠ "serving food." Dead zone between lunch and dinner (typically 3–7pm in Europe) must be verified against exact current time from `user_time_v0`. Failure already occurred at Postigo do Carvão at 5:40pm — cannot happen again.
- **v24 (2026-05-20):** Coastal city rule — always verify menu in seafood-heavy cities.
- **v23 (2026-05-20):** Seafood filter refined — include if alternatives exist, drop silently if not.
- **v22 (2026-05-20):** 🍽 dish row added to venue format.
- **v21 (2026-05-20):** Tour variety rule.
- **v20 (2026-05-20):** No review quotes on tours.
- **v19 (2026-05-20):** Tours full detail requirements.
