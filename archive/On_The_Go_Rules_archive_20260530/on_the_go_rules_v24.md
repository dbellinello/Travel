...
Created: 2026-04-30 | Updated: 2026-05-20 | Status: Active | Version: v24
...
| on\_the\_go\_rules (active) | `Rules/on_the_go_rules_v24.md` | this file |
...
4. **Seafood filter — always check the actual menu:**
   - Coastal cities (Porto, Lisbon, Cascais, Nice, Barcelona, etc.) are seafood-heavy by default. Never assume a restaurant is safe based on category alone.
   - Always verify the menu before including any restaurant. Search menu if needed.
   - If the menu has solid non-seafood options (steak, duck, lamb, pork, pasta, etc.) → include it, list only those dishes in the 🍽 row
   - If seafood dominates and meat options are minimal → drop it silently
   - Never warn Dani about seafood — just show what she can eat or skip it
...
- 🍽 Always include 2–4 key dishes Dani can eat. Search menu if needed. Only non-seafood (except at Japanese restaurants). In coastal cities, be extra careful — verify dishes are actually meat/non-seafood before listing.
...
- **Coastal city rule:** Porto, Lisbon, Cascais and similar coastal cities are seafood-heavy. Always check menus carefully. Never rely on restaurant category alone.
...
- **v24 (2026-05-20):** Coastal city rule added — Porto, Lisbon, Cascais and ocean-adjacent cities are seafood-heavy by default. Always verify actual menu before including any restaurant. Never rely on category. Extra scrutiny required near the coast.
- **v23 (2026-05-20):** Seafood filter — include if meat alternatives exist, drop silently if not. Never warn Dani.
- **v22 (2026-05-20):** Venue format — 🍽 row with 2–4 dishes per restaurant.
