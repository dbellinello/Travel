...
| on\_the\_go\_rules (active) | `Rules/on_the_go_rules_v12.md` | this file |
...
4. **Save new location** — if Dani mentions a new city or hotel in chat, note it for the session. Anchors all distance and Bolt/walk estimates. When city changes, run step 5 and clear previous city's gotchas.
...
- **Walk times:** Derived from the coordinates returned by `places_search` — calculate straight-line distance from hotel coords to venue coords, then estimate walking time at ~12 min/km. Show as `~N min walk`. Same method used for Bolt estimates. Walk row only appears when destination is plausibly walkable (<2.5 km); otherwise Bolt only.
- **Bolt/Uber times:** Estimated from distance using standard rates. International = Bolt; US = Uber.
...
- **Transport**: Walk, ride-app (Bolt/Uber), tram only — never metro/subway/bus
...
- **v12 (2026-05-20):** Walk times now estimated from `places_search` coordinates (same method as Bolt estimates) at ~12 min/km. Removed "tap Directions for walk time" — replaced with actual `~N min walk` estimates. Reservations rule hardened: never include reservation-required venues on any list, no exception. Bolt terminology used for international (was Uber).
- **v11 (2026-05-18):** Replaced static Current Location section with live sources: Google Calendar (primary) + Trips.html. Location is never hardcoded in this file again. Added Google Calendar to Available Tools.
- **v10 (2026-05-05):** Venue name now always a clickable link. Removed fake walk times. Added "coffee = espresso" to preferences. Updated hotel to Residence Inn Pasadena Old Town.
- **v9 (2026-05-05):** Unified venue format — single universal row order. Always show open+close time. 5 venues ordered closest to farthest.
