# Build state — Seoul
Started: 2026-05-30
Last updated: 2026-05-30

## Phase 0 — Session start
- [x] Rules for Claude.html

## Phase 1 — Pre-build orientation
- [x] Guide Structure.html
- [x] Stops Structure.html
- [x] Hotel Banner.html
- [x] Trip at a Glance.html
- [x] Toolbar.html
- [x] Navigation.html

## Phase 2 — Day shape
- [x] Day Structure.html

## Phase 3 — Per-stop build
- [x] Tickets.html
- [x] Motion Rule.html
- [x] Icon Order and Format.html
- [x] Photos Rules.html
- [x] Links.html

## Phase 4 — Per-section build
- [x] Weekly Closures - Extra Section.html
- [x] Tours - Extra Section.html
- [x] Cappuccino - Extra Section.html
- [x] Restaurants Near Hotel - Extra Section.html
- [x] Downtown Restaurants - Extra Section.html
- [x] Local Tastes - Extra Section.html
- [x] Food Delivery - Extra Section.html
- [x] Shows, Performances & Concerts - Extra Section.html
- [x] Getting Around - Extra Section.html
- [x] Train Stations Near Hotel - Extra Section.html
- [x] Day Trips by Train - Extra Section.html
- [x] Michelin Restaurants - Extra Section.html
- [x] Cities Gotchas - Extra Section.html (Seoul not in file — section omitted)

## Phase 5 — Ship gate
- [x] Brain/Reference/Ship Checklist.html
- [x] validate_itinerary.py passes — 688 passed · 0 failed · 6 warnings (all expected: day-count, cappuccino 4/5, RNH negative finding) — 2026-05-30
- [x] every extra populated or carries negative-finding line

## Notes — pickup session 2026-05-30
Tours: 5 Viator + 5 GYG + 4 TA confirmed (walking-tour max 4 respected). GYG t462225/t92337 review counts estimated from cross-platform data; Chrome MCP unavailable.
Cappuccino: 4/5 — no 5th confirmed within 25 min walk this build.
Restaurants Near Hotel: negative finding confirmed (0 qualifying within 25 min walk).
Downtown: 5 restaurants — Tosokchon · Myeongdong Kyoja · Woo Lae Oak · Gogung Insadong · Hadongkwan.
