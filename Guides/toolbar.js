 ...
 *
 * ⚠️ HOME: Travel/Guides/toolbar.js — PERMANENT. Both toolbar scripts
 * (toolbar.js + footnote.js) live inside Guides/ and must NEVER be moved
 * outside it. Every page loads them from Guides/:
 *   · Guides/City pages (depth 2):       src="../toolbar.js"
 *   · Guides/guides_index.html:          src="toolbar.js"
 *   · depth-1 pages (Trip Essentials/, On Demand/):  src="../Guides/toolbar.js"
 *   · depth-2 pages outside Guides (Trip Essentials/Maps/, Plug Adapter/):
 *                                        src="../../Guides/toolbar.js"
 *
 * Each page needs:
 *   <div id="toolbar-mount" data-depth="N" data-maxwidth="W"></div>
 *   <script src="PATH-TO-Guides/toolbar.js"></script>   ← before </body>
 *
...
(function () {
  ...
  styleEl.textContent =
    ...
    /* Scroll container — full viewport width; hides scrollbar; only scrolls
       horizontally if the button row is wider than the whole window. */
    '.tb-inner{overflow-x:auto;scrollbar-width:none}' +
    '.tb-inner::-webkit-scrollbar{display:none}' +
    /* Flex row — width:max-content keeps the row exactly as wide as the buttons
       (never capped to the content column, so it never left-packs with a gap and
       never clips the first button), and margin:0 auto centres that row in the
       viewport. The page content is also centred in the viewport, so the two share
       the same centre line regardless of font / emoji width. When the buttons are
       wider than the window (very narrow screens only) the auto margins collapse to
       0 and .tb-inner scrolls cleanly from the left. */
  ...
  if (!FOOTNOTE_RETIRED && !noFootnote) {
    ...
    /* footnote.js lives next to toolbar.js inside Guides/ — permanent home.
       base resolves to Travel/ root, so prefix with Guides/. */
    _fn.src = base + 'Guides/footnote.js';
