...
(function () {
  ...
  function build(db, fixedKey) {
    ...
    fab.innerHTML =
      '<svg viewBox="0 0 24 24" width="11" height="15" aria-hidden="true" ' +
      'style="vertical-align:-2px;margin-right:5px;">' +
      '<path fill="currentColor" d="M12 3a2 2 0 0 0-2 2v8.05A4 4 0 1 0 14 13V5a2 2 0 0 0-2-2z"/></svg>Weather';
    fab.addEventListener('keydown', function (e) {
      if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); open(); }
    });

    function placeInBanner() {
      /* Right-aligned inside the "Travel Guides" title banner — fills the empty
         space already there (no wasted line) and stays clear of the search box.
         White text to sit on the banner's coloured background. */
      ...
      if (header) {
        ...
        fab.style.cssText =
          'font-size:13px;font-weight:600;color:#fff;letter-spacing:.02em;' +
          'cursor:pointer;white-space:nowrap;margin-left:12px;transition:opacity .12s;';
        fab.addEventListener('mouseenter', function () { fab.style.opacity = '0.7'; });
        fab.addEventListener('mouseleave', function () { fab.style.opacity = '1'; });
    ...
    function placeFloating() {
      fab.style.cssText =
        'position:fixed;top:12px;right:16px;z-index:160;font-size:13px;font-weight:600;' +
        'color:' + GOLD + ';cursor:pointer;white-space:nowrap;transition:opacity .12s;';
      fab.addEventListener('mouseenter', function () { fab.style.opacity = '0.65'; });
      fab.addEventListener('mouseleave', function () { fab.style.opacity = '1'; });
    ...
    if (!placeInBanner()) {
      if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', function () {
          if (!placeInBanner()) placeFloating();
        });
      } else {
        placeFloating();
      }
    ...
    /* ── Per-guide hover — typical weather for a city when you hover its card ─
       Each index card links to a guide; the folder in its href is the data key.
       Hovering shows the current-month high/low + a compact 12-month strip. */
    function attachCardHovers() {
      ...
      tip.style.cssText =
        ...
        'font-family:inherit;color:' + INK + ';font-size:12px;';
      document.body.appendChild(tip);

      function content(rec) {
        var mo = new Date().getMonth(), u = unit();
        var hi = conv(rec.hi[mo]), lo = conv(rec.lo[mo]);
        var vals = rec.hi.concat(rec.lo);
        var lo0 = Math.min.apply(null, vals), hi0 = Math.max.apply(null, vals);
        var span = (hi0 - lo0) || 1, H = 30, bars = '';
        for (var j = 0; j < 12; j++) {
          var sel = (j === mo);
          var top = (1 - (rec.hi[j] - lo0) / span) * H;
          var bot = (1 - (rec.lo[j] - lo0) / span) * H;
          var bh = Math.max(2, bot - top);
          bars += '<div style="flex:1;position:relative;height:' + H + 'px;display:flex;justify-content:center;">' +
            '<div style="position:absolute;top:' + top + 'px;height:' + bh + 'px;width:4px;border-radius:2px;' +
            'background:' + (sel ? GOLD : 'rgba(138,108,26,.30)') + ';"></div></div>';
        }
        return '<div style="font-weight:700;color:' + GOLD + ';margin-bottom:1px;">' + (rec.city || '') + '</div>' +
          '<div style="font-size:10px;text-transform:uppercase;letter-spacing:.05em;color:' + MUTE + ';">' + MONTHS_LONG[mo] + '</div>' +
          '<div style="margin-top:3px;font-size:13px;">' +
            '<span style="color:#a61c00;font-weight:700;">' + hi + '°' + u + '</span> ' +
            '<span style="color:' + MUTE + ';font-size:11px;">high</span>&nbsp;&nbsp; ' +
            '<span style="color:#3d5282;font-weight:700;">' + lo + '°' + u + '</span> ' +
            '<span style="color:' + MUTE + ';font-size:11px;">low</span></div>' +
          '<div style="margin-top:5px;display:flex;align-items:flex-end;gap:1px;width:150px;">' + bars + '</div>';
      }
      function position(e) {
      ...
      [].forEach.call(cards, function (card) {
        ...
        var rec = db[folder];
        if (!rec || !rec.hi) return;
        card.addEventListener('mouseenter', function (e) { tip.innerHTML = content(rec); tip.style.display = 'block'; position(e); });
    ...
    function header() {
      var u = unit();
      var toggle =
        '<div style="display:inline-flex;border:1px solid ' + BORDER + ';border-radius:7px;overflow:hidden;flex-shrink:0;">' +
    ...
    function render() {
      panel.innerHTML =
        '<div style="padding:18px 20px 20px;">' + header() + body() +
